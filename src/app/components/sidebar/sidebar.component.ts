...
import { GetBreeds, GetCats } from '../../store/actions/app.actions'
...
export class SidebarComponent implements OnInit, OnDestroy{
  ...
  ngOnInit() {
    this.form.valueChanges.pipe(takeUntil(this.unsubscribe$)).subscribe(() => {
      this._store.dispatch(new GetCats({limit: this.form.get('limit')?.value, breed_ids: this.form.get('breed')?.value}))
    })
    this.breeds$.pipe(takeUntil(this.unsubscribe$)).subscribe(breeds => {
      this.createForm(breeds[0])
    })
